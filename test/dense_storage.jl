...
@testset "HDF5Vectors dense storage" begin
    ...
    mktempdir() do directory
        ...
        HDF5.h5open(joinpath(directory, "dense_storage.h5"), "w") do file
            ...
            for (name, schema, values) in cases
                ...
                initial_batch = HDF5Vectors.encode_batch(schema, initial_values)
                HDF5Vectors.initialize_encoded!(store, initial_batch)
                final_frame = encode_value(schema, last(values))
                HDF5Vectors.append_encoded!(store, length(values), final_frame)
...
@testset "HDF5Vectors dense batch and validation" begin
    ...
    mktempdir() do directory
        ...
        HDF5.h5open(joinpath(directory, "dense_validation.h5"), "w") do file
            ...
            direct_store = HDF5Vectors.create_store(
                direct_group,
                direct_schema;
                chunk_length = 2,
            )
            direct_batch = HDF5Vectors.encode_batch(direct_schema, direct_values)
            HDF5Vectors.initialize_encoded!(direct_store, direct_batch)
            stored_batch = HDF5Vectors.read_encoded_batch(direct_store, 1:2)

            @test stored_batch == direct_batch
            @test HDF5Vectors.decode_batch(direct_schema, stored_batch) == direct_values
            ...
            empty_batch = HDF5Vectors.encode_batch(direct_schema, direct_values[1:0])
            @test size(empty_batch) == (2, 0)
            HDF5Vectors.initialize_encoded!(empty_store, empty_batch)
            @test size(HDF5Vectors.read_encoded_batch(empty_store, 1:0)) == (2, 0)
            @test iszero(HDF5Vectors.physical_length(empty_store))
